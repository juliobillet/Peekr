...
const howItWorks = [
  ...
  },
  {
    ...
    description:
      "Defina sua disponibilidade e personalize a duração e o valor das sessões.",
  ...
  },
  {
    ...
    description:
      "Cliente e especialista recebem o e-mail e o evento nos seus calendários.",
...
const audienceCards = [
  ...
  { title: "Prática guiada", icon: "/icons/pin-icon.png" },
  {
    ...
    icon: "/icons/profile-big-arrow-icon.png",
  },
];
      ...
      <section className="grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
        <div className="space-y-6 pt-6">
          ...
          <p className="max-w-xl text-lg/relaxed text-[#0c1b58] sm:text-2xl/relaxed">
            Peekr reúne perfil, agenda e confirmação para aulas, mentorias,
            revisões e consultorias privadas ao vivo. Você ensina, orienta e
            transforma — nós cuidamos do resto.
          </p>
          <div className="flex flex-wrap gap-3">
            <Link
              href="/register"
              className="rounded-xl bg-[#0b4bff] px-8 py-4 text-lg font-semibold text-white"
            >
              Criar meu perfil
            </Link>
            <Link
          ...
          <div className="grid max-w-2xl grid-cols-1 gap-4 pt-2 sm:grid-cols-3 text-[#0d1b57]">
            <div className="flex items-center gap-2">
              <span>👥</span>
              <span>
                <b>+10 mil</b> profissionais
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span>📅</span>
              <span>
                <b>+120 mil</b> sessões realizadas
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span>🛡️</span>
        ...
        <div className="relative pb-28 lg:pb-20">
          ...
          <img
            ...
            alt="Reserva e confirmação"
            className="absolute -bottom-6 right-3 w-[40%]"
          />
        </div>
      </section>

      <p className="text-center text-lg text-[#5b6a91]">
        ✓ Agendamento confirmado por e-mail e registrado nos calendários.
      </p>

      <section className="space-y-10 pt-2">
        <h2 className="text-center text-3xl font-bold sm:text-5xl text-[#0d1b57]">
          Como funciona
        </h2>
          ...
          {howItWorks.map((item, index) => (
            <article key={item.step} className="space-y-4 text-center">
              <div className="relative mx-auto flex h-16 w-16 items-center justify-center rounded-2xl border border-[#d9e2fa] bg-white shadow-sm">
                ...
                />
                <span className="absolute -bottom-2 -left-2 rounded bg-[#0b4bff] px-1.5 py-0.5 text-xs font-bold text-white">
                  {index + 1}
                </span>
              </div>
              <h3 className="text-2xl font-semibold text-[#0d1b57]">
                {item.title}
              </h3>
      ...
      <section className="space-y-8">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
          {audienceCards.map((item) => (
            <article
              key={item.title}
              className="flex min-h-40 flex-col items-center justify-center gap-4 rounded-2xl border border-[#e3e9fb] bg-white p-4 text-center"
            >
              ...
              <h3 className="text-xl font-semibold leading-tight text-[#0d1b57]">
                {item.title}
              </h3>
            </article>
          ))}
        </div>
        <p className="text-center text-2xl text-[#5c6d94]">
          Educação, carreira, idiomas, negócios, tecnologia e muito mais.
        </p>
      </section>
      ...
      <section className="rounded-3xl bg-[#f3f6ff] p-8">
        <div className="grid items-center gap-6 lg:grid-cols-[1fr_1.2fr]">
          <div className="space-y-4">
            <h2 className="text-3xl font-bold text-[#0d1b57] sm:text-5xl">
              Pronto para transformar sua presença em impacto?
            </h2>
            <p className="text-2xl text-[#5c6d94]">
              Crie seu perfil gratuitamente e comece a receber seus primeiros
              agendamentos.
            </p>
            <div className="flex flex-col gap-3 sm:flex-row">
              <Link
                href="/register"
                className="rounded-xl bg-[#0b4bff] px-6 py-3 text-lg font-semibold text-white"
              >
                Criar meu perfil
              </Link>
              <Link
                href="/athlon"
                className="rounded-xl border border-[#d4ddf8] bg-white px-6 py-3 text-lg font-semibold text-[#0d1b57] transition hover:text-brand-orange"
              >
                Ver exemplo de perfil
              </Link>
            </div>
          ...
          <img
            ...
            alt="Visão geral e próxima sessão"
            className="w-full"
          />
        </div>
      </section>
