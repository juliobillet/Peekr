  ...
  return (
    <div className="space-y-16">
      <section className="space-y-6 py-10">
        ...
        <p className="max-w-3xl text-zinc-300">A Peekr reúne perfil, agenda, pagamento e sala efêmera para aulas, mentorias, revisões e consultorias privadas ao vivo.</p>
      ...
      <section className="space-y-6">
        ...
        <div className="grid gap-3 md:grid-cols-4">{steps.map((s, i) => <Card key={s}><CardContent><p className="text-sm text-brand-orange">0{i + 1}</p><p className="mt-2 font-medium">{s}</p></CardContent></Card>)}</div>
      </section>
      ...
      <section className="space-y-4 rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6">
        ...
        <p className="text-sm text-zinc-400">A Peekr também não promete bloqueio absoluto contra gravações externas feitas por softwares ou dispositivos de terceiros.</p>
      </section>

      <section className="rounded-2xl border border-zinc-800 bg-zinc-900/60 p-8 text-center">
